  ...
  const sendEmailCode = async () => {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        setEmailState(s => ({ ...s, sending: false, sent: false, msg: "전송 실패" }));
      }
    } catch {
  ...
  const verifyEmailCode = async () => {
    ...
    try {
      ...
      if (data.code === 200) {
        ...
        setEmailState(s => ({ ...s, verifying: false, verified: false, msg: "인증 실패" }));
      }
    } catch {
      ...
      <form className="loginBox" onSubmit={handleSubmit}>
        ...
        <div className="container" style={{ display: "flex", gap: 8 }}>
          <input type="email" name="email" placeholder="이메일" value={form.email}
            onChange={handleChange} readOnly={emailState.verified} required />
