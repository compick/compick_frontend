...
export const getEplMatches = async (startISO, endISO) => {
  ...
  const { data } = await api.get('/soccer/epl/match', {
...
export const getEplMatchesMonthly = async (year, month) => {
  ...
  const { data } = await api.get('/soccer/epl/match/monthly', {
    params: { year, month },
  });
  return data;
};
...
export const getLaligaMatchesMonthly = async (year, month) => {
  ...
  const { data } = await api.get('/soccer/laliga/match/monthly', {
    params: { year, month },
  });
  return data;
};
...
/** 축구 - 전체 리그 경기 조회 */
export const getAllSoccerMatches = async (startISO, endISO) => {
  console.log('Calling getAllSoccerMatches with:', { startISO, endISO });
  const { data } = await api.get('/soccer/all/match', {
    params: { start: startISO, end: endISO },
  });
  return data;
};
...
export const getAllSoccerMatchesMonthly = async (year, month) => {
  ...
  const { data } = await api.get('/soccer/all/match/monthly', {
    params: { year, month },
  });
  return data;
};
...
export const getEplRankings = async (season = new Date().getFullYear()) => {
  ...
  const { data } = await api.get('/soccer/epl/rank', {
    params: { season },
  });
  return data;
};
...
export const getLaligaRankings = async (season = new Date().getFullYear()) => {
  ...
  const { data } = await api.get('/soccer/laliga/rank', {
    params: { season },
  });
  return data;
};
