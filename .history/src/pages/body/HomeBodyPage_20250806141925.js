...
export default function HomeBodyPage() {
 const [selectedTab, setSelectedTab] = useState("전체");
  ...
  //날짜 기준 그룹핑 로직
  const groupByDate = matches.reduce((acc, cur) => {
  ...
  Object.entries(groupByDate)
    .sort((a, b) => a[0].localeCompare(b[0])) // 날짜 정렬
    .map(([date, matches]) => (
      <div key={date}>
              ...
              Object.entries(groupByLeague).map(([leagueName, matches]) => {
                ...
                return (
                  <div key={leagueName}>
                    ...
                    <h3 className="leagueTitle">
                      <img src={leagueLogo} className="leagueLogo" alt={leagueName} />
                      {leagueName}
                    </h3>
                    {sortedMatches.map((match, idx) => {
                      const showDateHeader = match.date !== prevDate;
...
export function getLeagueLogo(leagueName) {
  if (leagueName === "England Premium League") return eplLogo;
  if (leagueName === "LA LIGA EA SPORTS") return laligaSpain;
  if (leagueName === "UEFA Champions League") return championLogo;
  return null; // 없는 경우 처리
}